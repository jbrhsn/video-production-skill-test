...
model_cache = load("model_cache.py")
...
class PipelineTests(unittest.TestCase):
    ...
    def test_audio_duration_includes_trailing_silence(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            assets = Path(directory) / ".video_production_assets"
            assets.mkdir()
            args = type("Args", (), dict(audio=str(path), out=str(output), model="base",
                                         model_dir=None, assets_dir=str(assets), language="en"))()
    ...
    def test_model_cache_hashes_existing_files_without_downloading(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            payload = b"verified model fixture"
            manifest = {"kokoro": {}, "whisper": {"tiny": {
                "filename": "tiny.pt", "url": "https://example.invalid/tiny.pt",
                "sha256": __import__("hashlib").sha256(payload).hexdigest()}}}
            target = root / ".video_production_assets/whisper/tiny.pt"
            target.parent.mkdir(parents=True)
            target.write_bytes(payload)
            with patch.object(model_cache, "load_manifest", return_value=manifest), patch.object(model_cache, "download") as download:
                rows = model_cache.ensure(root / ".video_production_assets", ["whisper:tiny"], allow_download=True)
            self.assertEqual(rows[0]["status"], "cached")
            download.assert_not_called()

    def test_model_cache_rejects_corrupt_file_without_ensure(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            manifest = {"kokoro": {}, "whisper": {"tiny": {
                "filename": "tiny.pt", "url": "https://example.invalid/tiny.pt", "sha256": "0" * 64}}}
            target = root / ".video_production_assets/whisper/tiny.pt"
            target.parent.mkdir(parents=True)
            target.write_bytes(b"corrupt")
            with patch.object(model_cache, "load_manifest", return_value=manifest):
                rows = model_cache.ensure(root / ".video_production_assets", ["whisper:tiny"], allow_download=False)
            self.assertEqual(rows[0]["status"], "missing-or-invalid")
