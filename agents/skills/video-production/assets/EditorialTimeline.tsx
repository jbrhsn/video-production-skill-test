...
const audioTrimFrame = (clip: Clip, outputFps: Fps) => Math.round((clip.sourceRangeUs?.[0] ?? 0) * fpsNumber(outputFps) / 1_000_000);
const canvas = (DESIGN_TOKENS as {colors?: {background?: string}}).colors?.background ?? "#101418";

const SourceVideo: React.FC<{clip: Clip}> = ({clip}) => clip.freezeFrameUs === null || clip.freezeFrameUs === undefined ? <Video src={staticFile(clip.src!)} trimBefore={sourceFrame(clip)} playbackRate={rate(clip)} muted name={`${clip.role}:${clip.id}`} objectFit="cover" style={{width: "100%", height: "100%"}} /> : <Img src={staticFile(clip.freezeSrc!)} style={{width: "100%", height: "100%", objectFit: "cover"}} />;
const SourceAudio: React.FC<{clip: Clip; fps: Fps}> = ({clip, fps}) => clip.pitchPolicy === "mute" ? null : <Audio src={staticFile(clip.src!)} trimBefore={audioTrimFrame(clip, fps)} playbackRate={rate(clip)} />;
...
export const EditorialTimeline: React.FC<{data: EditorialTimelineData; masterStart?: number; showSafeArea?: boolean}> = ({data, masterStart = 0, showSafeArea = false}) => {
  ...
  return <AbsoluteFill style={{background: canvas, overflow: "hidden"}}>
    ...
    {audio.map(clip => <Sequence key={clip.id} from={clip.startFrame - masterStart} durationInFrames={clip.durationFrames} layout="none"><SourceAudio clip={clip} fps={data.fps} /></Sequence>)}
