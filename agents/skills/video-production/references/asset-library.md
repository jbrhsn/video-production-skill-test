...
After acceptance, add selected output media to `asset-manifest.json`. This is the machine-readable register for staged visual and audio dependencies: stable ID, kind, `public/media/...` staged path, original source, usable rights status and basis, attribution, accepted status, inspection note, and optional SHA-256. The Markdown plan retains requests and decisions; do not maintain a competing machine status there. Music and SFX additionally record useful source region, technical duration and listening result. Presence in the manifest does not replace human suitability review.
