...
def resolve_style(catalog, request, brand=None, overrides=None):
    if not isinstance(catalog, dict) or catalog.get("version") != 1 or not isinstance(catalog.get("profiles"), dict):
        raise ValueError("style catalog requires version 1 and profiles")
    ...
    if brand is not None:
        if not isinstance(brand, dict) or set(brand) - {"tokens", "prohibited"}:
            raise ValueError("brand permits only tokens and prohibited")
    ...
    payload = {"schema": "resolved-style", "version": 1, "profile": profile_id,
