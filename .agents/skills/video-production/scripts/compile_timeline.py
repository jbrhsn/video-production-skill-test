...
def main():
    ...
    parser.add_argument("--visual-events")
    parser.add_argument("--out", required=True)
    args = parser.parse_args()
    timeline = json.loads(Path(args.timeline).read_text(encoding="utf-8"))
    sources = json.loads(Path(args.sources).read_text(encoding="utf-8"))
    events = json.loads(Path(args.visual_events).read_text(encoding="utf-8")) if args.visual_events else None
    compiled = compile_timeline(timeline, sources, events)
