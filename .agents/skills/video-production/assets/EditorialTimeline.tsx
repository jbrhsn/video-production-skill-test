...
type Clip = {id: string; trackId: string; role: string; kind: "audio" | "video" | "generated"; startFrame: number; durationFrames: number; zIndex: number; src?: string; sourceRangeUs?: [number, number]; sourceFrameRate?: Fps | null; playbackRate?: Fps; freezeFrameUs?: number | null; pitchPolicy?: "preserve" | "follow-rate" | "mute"; generator?: string; payload?: Record<string, unknown>};
...
const SourceVideo: React.FC<{clip: Clip}> = ({clip}) => <Video src={staticFile(clip.src!)} trimBefore={sourceFrame(clip)} freezeFrame={clip.freezeFrameUs === null || clip.freezeFrameUs === undefined ? undefined : 0} playbackRate={rate(clip)} muted name={`${clip.role}:${clip.id}`} style={{width: "100%", height: "100%", objectFit: "cover"}} />;
