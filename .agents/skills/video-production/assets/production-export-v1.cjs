...
const review = spawnSync('uv', ['run', '--no-project', '--python', config.python, 'python',
  path.join(__dirname, 'production/check_production_v1.py'), '--project-dir', root,
  '--project', config.project, '--state', config.state], {cwd: root, stdio: 'inherit'});
if (review.error || review.status !== 0) process.exit(review.status || 1);
const qc = spawnSync('uv', ['run', '--no-project', '--python', config.python, 'python',
  path.join(__dirname, 'production/run_qc.py'), '--timeline', path.join(root, 'src/timeline-data.json'),
  '--out', path.join(root, 'qc/render-timeline.json')], {cwd: root, stdio: 'inherit'});
