...
Inventory creative media under WORKSPACE/.video_production_assets: images, B-roll, videos, and sound effects. For a reusable library or new intake, read [asset library](references/asset-library.md); it distinguishes curated source/rights metadata from a generated technical index. Before preparing a storyboard, search the curated inventory with topic and visual-beat terms, inspect suitable candidates, and list only selected real assets in the storyboard. Candidate search is an input to art direction, not a requirement to reuse something; unavailable, unsuitable, or uncleared assets never constrain the video's scale or visual treatment. Existing kokoro/ and whisper/ subdirectories are runtime model caches, not creative media. Keep them separate from the creative inventory. Missing media never limits the work to text slides: create original animation, motion graphics, diagrams, illustrated action, or spatial visual metaphors in code. Use media when it strengthens the explanation, not to fill an asset quota.
