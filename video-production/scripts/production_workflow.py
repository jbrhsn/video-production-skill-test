...
def check_export(project_dir, project_path=None, state_path=None):
    ...
    review_path = project["artifacts"].get("assetLibraryReview")
    if not review_path:
        raise ValueError("canonical project requires an assetLibraryReview artifact before export")
    validate_review(root / review_path, {project["route"]})
